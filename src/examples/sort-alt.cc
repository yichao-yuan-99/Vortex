...
using gpuio::execution::ops::sort::groupForMerge;
using gpuio::execution::ops::sort::MergeLevelsOp;
...
int main() {
  ...
  std::vector<std::vector<size_t>> mg;
  r = gpuio::utils::time::timeit([&] {
    mg = groupForMerge(hostDBuf.current(), input_size, chunk_size, chunk_size);
  });

  fmt::print("meta, {}\n", r);

  r = gpuio::utils::time::timeit([&] {
    StaticPipelineExecutor<MergeLevelsOp<dtype>> exec(layout, hostDBuf.current(), hostDBuf.alternate(), chunk_size, mg);
    exec.run(exchange, s);
    // hostDBuf.swap();
  });
  fmt::print("merge, {}\n", r);
