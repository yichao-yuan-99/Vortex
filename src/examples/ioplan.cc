...
std::vector<double> test(size_t totalTraffic, size_t granularity, size_t fraction, HostVector<uint32_t> &groundTruth, HostVector<uint32_t> &answer, int repeat) {
  ...
  gpuio::sched::forwardingStage stage(granularity);
  gpuio::sched::profile::PlanContext context(dstDevice, srcHost, dstHost, srcDevice, stage);

  for (int i = 0; i < repeat + 1; i++) { // 0 will be warmup
    ...
    gpuio::sched::profile::NaivePlanGenerator plan(context);
    gpuio::sched::profile::BlockPlanExecutor exec(context, plan);
    ...
    auto t = gpuio::utils::time::timeit([&] {
      // exec.lanuch();
      // exec.lanuch_s(); // rely on hip stream to sync
      // exec.lanuch_ss(); // rely on hip stream and event to sync
      exec.lanuch_gg(); // Most careless schedule
      // exchange.launch();
      // exchange.sync();
    });
    auto bw = (H2DTraffic + D2HTraffic) / 1000'000'000.0/ t;
    if (i) bandwidthResults.push_back(bw); // ignore the first test
...
// ./ioplan <totalTraffic> <granularity> <repeat>
int main(int argc, char **argv) {
  argparse::ArgumentParser program("ioplan");
  ...
  program.add_description("test the bandwidth of statically scheduled IO.");
  ...
  // std::vector<size_t> fractions = {6};
  // std::vector<size_t> fractions = {7};
  ...
  // size_t H2DTraffic = 4000'000'000, D2HTraffic = 4000'000'000, bufferSize = 20'000'000;
  // gpuio::hip::DeviceMemory dstDevice, srcDevice;
  // {
  //   gpuio::hip::DeviceGuard on(0);
  //   dstDevice = gpuio::hip::DeviceMemory(H2DTraffic);
  //   srcDevice = gpuio::hip::DeviceMemory(D2HTraffic);
  // }
  // gpuio::hip::HostVector<uint32_t> srcHost(H2DTraffic / sizeof(uint32_t));
  // gpuio::hip::HostVector<uint32_t> dstHost(D2HTraffic / sizeof(uint32_t));

  // gpuio::sched::forwardingStage stage(bufferSize);
  // gpuio::sched::profile::PlanContext context(dstDevice, srcHost, dstHost, srcDevice, stage);
  ...
  //     exec.lanuch();
  //   });
  //   auto totalTraffic = H2DTraffic + D2HTraffic;
  //   fmt::print("takes: {:.5f}, bw: {:.3f} GB/s\n", t, totalTraffic / 1000'000'000.0/ t);
  // }
  // gpuio::sched::profile::BlockPlanExecutor exec(context, plan);
