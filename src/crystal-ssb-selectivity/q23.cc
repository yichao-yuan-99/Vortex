...
struct CrystalQ23Op {
  ...
  CrystalQ23Op(void *, size_t, ssb::DeviceBuffers &buf, size_t chunk_cnt, ssb::Dataset &data)
  ...
  int operator()(MemoryRef mem, int, int it, hipStream_t s) {
    ...
    crystal::q2x::probleTables<256, 4>(
      p_lo_orderdate, layout.lo_partkey, p_lo_suppkey, p_lo_revenue,
      inputsCnts_[it],
...
int main() {
  ...
  { // perform computation
    ...
    t = gpuio::utils::time::timeit([&] {
      ...
      gpuio::execution::StaticPipelineExecutor<CrystalQ23Op> exec(layout, bufs, chunk_cnt, data);
      exec.run(exchange, s);
    });
    fmt::print("exec, {}\n", t);
