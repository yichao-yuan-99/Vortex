...
#include <crystal-zero-copy/ssb/q34.h>
...
struct CrystalQ34Op {
  ...
  CrystalQ34Op(void *, size_t, ssb::DeviceBuffers &buf, size_t chunk_cnt, ssb::Dataset &data)
  ...
  int operator()(MemoryRef mem, int, int it, hipStream_t s) {
    ...
    crystal::q34::probleTables<256, 4>(
...
int main() {
  fmt::print(fmt::emphasis::bold | fmt::fg(fmt::color::green), "SSB QEURY 3.4\n");
  ssb::Dataset data("/home1/yichaoy/work/dbGen/crystal/test/ssb/data/s1000_columnar", ssb::Q34DataConfig);
  ...
  { // perform computation
    ...
    t = gpuio::utils::time::timeit([&] {
      ...
      crystal::q34::buildTables<256, 4>(
        bufs.d_datekey, bufs.d_year, bufs.d_yearmonthnum, ssb::d_total_num,
      ...
      gpuio::execution::StaticPipelineExecutor<CrystalQ34Op> exec(layout, bufs, chunk_cnt, data);
      exec.run(exchange, s);
    });
    fmt::print("exec, {}\n", t);
