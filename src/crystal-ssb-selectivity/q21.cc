...
struct CrystalQ21Op {
  ...
  CrystalQ21Op(void *, size_t, ssb::DeviceBuffers &buf, size_t chunk_cnt, ssb::Dataset &data)
  ...
  int operator()(MemoryRef mem, int, int it, hipStream_t s) {
    ...
    crystal::q2x::probleTables<256, 4>(
      p_lo_orderdate, layout.lo_partkey, layout.lo_suppkey, p_lo_revenue,
      inputsCnts_[it],
...
int main() {
  fmt::print(fmt::emphasis::bold | fmt::fg(fmt::color::green), "SSB QEURY 2.1\n");
  ssb::Dataset data("/home1/yichaoy/work/dbGen/crystal/test/ssb/data/s1000_columnar", ssb::Q21DataConfig);
  ...
  { // perform computation
    ...
    t = gpuio::utils::time::timeit([&] {
      ...
      crystal::q2x::buildTables<256, 4, 1>(
      ...
      gpuio::execution::StaticPipelineExecutor<CrystalQ21Op> exec(layout, bufs, chunk_cnt, data);
      exec.run(exchange, s);
    });
    fmt::print("exec, {}\n", t);
