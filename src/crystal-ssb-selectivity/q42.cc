...
struct CrystalQ42Op {
  ...
  CrystalQ42Op(void *, size_t, ssb::DeviceBuffers &buf, size_t chunk_cnt, ssb::Dataset &data)
  ...
  int operator()(MemoryRef mem, int, int it, hipStream_t s) {
    ...
    int *p_lo_partkey = static_cast<int *>(data_.lo_partkey) + offset;
    ...
    crystal::q42::probleTables<256, 4>(
      p_lo_orderdate, p_lo_partkey, layout.lo_custkey, layout.lo_suppkey, 
...
int main() {
  ...
  { // perform computation
    ...
    t = gpuio::utils::time::timeit([&] {
      ...
      gpuio::execution::StaticPipelineExecutor<CrystalQ42Op> exec(layout, bufs, chunk_cnt, data);
      exec.run(exchange, s);
    });
    fmt::print("exec, {}\n", t);
