...
struct CrystalQ41Op {
  ...
  int operator()(MemoryRef mem, int, int it, hipStream_t s) {
    ...
    crystal::q41::probleTables<256, 4>(
      p_lo_orderdate, layout.lo_partkey, layout.lo_custkey, layout.lo_suppkey, 
...
int main() {
  ...
  { // perform computation
    ...
    t = gpuio::utils::time::timeit([&] {
      ...
      gpuio::execution::StaticPipelineExecutor<CrystalQ41Op> exec(layout, bufs, chunk_cnt, data);
      exec.run(exchange, s);
    });
    fmt::print("exec, {}\n", t);
