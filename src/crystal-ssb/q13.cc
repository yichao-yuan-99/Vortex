...
struct CrystalQ13Op {
  ...
  CrystalQ13Op(void *, size_t, ssb::DeviceBuffers &buf, size_t chunk_cnt, ssb::Dataset &data)
  ...
  int operator()(MemoryRef mem, int, int it, hipStream_t s) {
    ...
    crystal::q1x::Q13Run<256, 4>(layout.lo_orderdate, layout.lo_discount, layout.lo_quantity, layout.lo_extendedprice,
...
int main() {
  fmt::print(fmt::emphasis::bold | fmt::fg(fmt::color::green), "SSB QEURY 1.3\n");
  ...
  { // perform computation
    ...
    t = gpuio::utils::time::timeit([&] {
      ...
      gpuio::execution::StaticPipelineExecutor<CrystalQ13Op> exec(layout, bufs, chunk_cnt, data);
      exec.run(exchange, s);
    });
    fmt::print("exec, {}\n", t);
